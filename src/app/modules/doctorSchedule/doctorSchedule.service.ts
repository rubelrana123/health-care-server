...
const getMySchedules = async (user: IJWTPayload, filters: any, options: IOptions) => {
    const { limit, page, skip } = paginationHelper.calculatePagination(options);
...
const deleteFromDB = async (user: IJWTPayload, scheduleId: string) => {
    ...
    if (isBookedSchedule) {
        throw new Error( "You can not delete the schedule because of the schedule is already booked!")
    }
