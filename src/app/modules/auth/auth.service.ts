...
import emailSender from './emailSender';
 ...
 const refreshToken = async (token: string) => {
    let decodedData;
    try {
        //give decoded data example: { email: "x@gmail.com", role: "ADMIN", iat: 1696543200, exp: 1699121600 }
...
const forgotPassword = async (payload: { email: string }) => {
    ...
    const resetPassLink = config.reset_pass_link_client_url + `?userId=${userData.id}&token=${resetPassToken}`
